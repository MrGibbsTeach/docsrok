...
In ${b.state}, this obligation is reinforced by specific psychosocial risk regulations${b.state === 'NSW' ? ' (WHS Regulation 2017 (NSW) Part 3.7A, effective September 2022)' : b.state === 'QLD' ? ' (WHS Regulation 2011 (Qld) Part 5.1A, effective April 2023)' : ''}.
...
const ACTIVITY_GUIDANCE: Record<string, string> = {
  working_at_heights:
    'Falls from height are the #1 cause of construction fatalities. The WHS Regulation (s.78, both QLD and NSW) requires fall prevention for any work at a height of 2 metres or more. Cover: the 2-metre trigger and what fall prevention is required (edge protection, scaffolding, EWPs); hierarchy of controls for fall prevention (passive collective protection preferred over personal fall arrest); EWPs (elevated work platforms) including operator competency; fall arrest harnesses — correct selection, fit, inspection, and rescue-from-height procedures (rescue plan mandatory before donning harness); ladder safety (never use as a working platform for extended periods); roof work and skylights; penetrations/holes must be covered and secured or barricaded. Reference Safe Work Australia Code of Practice: Managing the Risk of Falls at Workplaces.',
  manual_handling:
  ...
  concrete_cutting:
    'Cover: crystalline silica dust controls — the national workplace exposure standard for respirable crystalline silica (RCS) is 0.05 mg/m³ TWA (reduced from 0.1 mg/m³ in 2020). Hierarchy of controls for silica: elimination (avoid silica-generating tasks where possible), engineering controls (wet cutting mandatory, on-tool extraction / LEV), administrative controls (exposure time limits, air monitoring), PPE (P2 RPE as last resort only — not standalone control). Health surveillance requirements for workers regularly exposed. Noise controls (exposure standard 85 dB(A) TWA / 140 dB(C) peak — reference Safe Work Australia Code of Practice: Managing Noise and Preventing Hearing Loss at Work). Blade safety (guards, correct blade for material). Hand-arm vibration (HAVS) — HAV exposure action value 2.5 m/s² A(8). Concrete slurry classified as hazardous waste — containment and licensed disposal. Reference Safe Work Australia Code of Practice: Engineered Stone.',
  scaffolding:
  ...
  crane_rigging:
    'Cover: High Risk Work (HRW) licence classes — DG (Dogging), RI/RB/RA (Rigging intermediate/basic/advanced), CV/CN/CO/CB/CA (Crane operator classes). Critical lift definition: load exceeds 85% of cranes rated capacity, OR load is handled by two or more cranes, OR load is over energised services, OR persons are within the crane\'s operating radius — formal written lift plan required for all critical lifts. Exclusion zones (1.5× load height minimum, or as per lift plan). Communication — banksman hand signals (AS 2550) and radio backup. Ground bearing capacity assessment, outrigger bearing pads and mats. SWL of every item in the lifting assembly (crane, chain blocks, slings, shackles, spreader bars) — weakest link governs. Daily pre-use inspection records. Plant registration requirements for cranes — confirm registration currency before use.',
  demolition:
